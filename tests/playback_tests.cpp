...
protected:
    ...
    void SetUp() override
    {
        ...
        try {
            m_soundManager.initialize(TestAudioFiles::s_testRootDir.string(), 8, 32);
        } catch(...) {
            // If initialization fails, tests will check isInitialized()
        }
    }
...
{
    ASSERT_TRUE(m_soundManager.isInitialized());
